...
def analyze_image(image_path):
    try:
        ...
        response = model.generate_content(["On this image is a math problem if not say ONLY 'error 1'(with out ') if there is say it as a python list so 1 awnser needed(example): 13 and if for examople 2: 15, 94 and if its with remainder(for example): 12 remainder: 2. and if its 2 times with remainder(for example): 12 remainder: 2, 15 remainder: 3,", img])
        response.resolve()
